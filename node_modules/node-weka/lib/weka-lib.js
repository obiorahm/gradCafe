  ...
  exports.correlationMatrix = function (data, options, cb) {
    ...
    async.waterfall([
      ...
      function (fileIdTraining, callback) {
        ...
        child = exec('java -classpath /tmp/node-weka/bin/weka.jar ' + options.evaluator + ' '
            + options.e_params +
            ' -s "' + options.ranker + '"' +
            ' -i ' + fileIdTraining, {cwd: __dirname},
  ...
  exports.classify = function (data, test, options, cb) {
    ...
    async.waterfall([
      ...
      console.log('weka ' + options.classifier + ' '
        ...
        ' -T ' + fileIdTest +
        ' -no-cv -v -p' + 0);
            console.log("print the directory " + __dirname);
            pre_dir = __dirname
            var n = pre_dir.substring(0, pre_dir.search("lib")) + 'bin/weka.jar';
        child = exec('java -classpath ../bin/weka ' + options.classifier + ' '
            + options.params +
            ' -t ' + fileIdTraining +
            ' -T ' + fileIdTest +
            ' -no-cv -v -p 0', {cwd: __dirname},
          function (error, stdout, stderr) {
